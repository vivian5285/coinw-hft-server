...
2026-09-19新增(CoinW侧)：原样复制自eth-webhook-server-main/strategy_
engine/klines.py，作为跟币安B系统保持字节一致的共享工具模块——宝贝
拍板"综合硬止损/双均线破位/反转锁利/趋势确认重入"这些风险计算类K线
统一改拉币安公开数据，CoinW自己的K线只做兜底(实盘复现：OPENAI/XPD/
XAU两边算出来的综合硬止损经常对不上，根源是CoinW自己的行情深度比
币安薄，偶尔一笔稍大的单子就能在CoinW自己的K线上制造一个"假摆动点"）。
仓位查询/下单/止盈止损这些执行类操作完全不受影响，还是CoinW自己的
coinw_client.py。以后币安那边改进这个模块的bug修复，同步复制过来。

跟账户交易代码完全隔离：只打币安公开行情端点（无需 API Key、无需签名），
不依赖CoinW任何账户/交易代码——这里服务的是全部品种，理应独立。
