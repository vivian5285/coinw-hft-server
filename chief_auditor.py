...
# -*- coding: utf-8 -*-
"""
督察官复查清单 - CoinW单系统 v16.22.1

硬失败(触发暂停): 方向、TP切片≈30%、硬止损缺失
软警告(只告警): 杠杆、权重、雷达、API频率
"""
...
class AuditResult:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "items": [
                {"key": i.key, "ok": i.ok, "hard": i.hard, "detail": i.detail}
                for i in self.items
            ],
        }
...
def audit_open_bundle(facts: Dict[str, Any]) -> AuditResult:
    ...
    risk_cfg = _f(facts.get("risk_pct_cfg", 0.20))
    if risk_cfg <= 0:
        risk_cfg = 0.20
    ...
    lev_cap = _f(facts.get("leverage_cap", 5))
    if lev_cap <= 0:
        lev_cap = 5.0
    ...
    items.append(AuditItem(
        "api_freq", api_ok, False,
        f"recent={recent} budget={budget}",
    ))
