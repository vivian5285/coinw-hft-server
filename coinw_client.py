...
功能：封装资产查询、盘口现价、全域撤单、真市价开仓、持仓 ID 绑定限价单及循环平仓绞杀。
"""
...
class CoinWClient:
    ...
    def place_market_order(self, symbol, side, amount, leverage=20):
        """
        【完美修复版】真·市价开仓委托
        彻底剥离价格参数，加入合规的 positionType: "1"，强制撮合引擎执行市价 Taker 吃单。
        """
        return self._request("POST", "/v1/perpum/order", {
            ...
            "positionType": "1"            # 【关键修复】1 代表普通市价单，彻底封杀 902 报错
        })
    ...
    def cancel_all_open_orders(self, symbol="ETH"):
        ...
        return {"code": 0, "msg": f"全域撤单完毕：已成功从盘口抹去 {cancel_count} 笔遗留单"}#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
ETH 万亿战神 AI 量化交易引擎 - 币赢 (CoinW) 私有核心通信客户端
功能：封装资产查询、盘口现价、全域撤单、真市价开仓、持仓 ID 绑定限价单及循环平仓绞杀。
